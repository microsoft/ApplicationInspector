namespace ApplicationInspector.Unitprocess.CLICommands
{
    ...
    public class CLITestExportTagsCmd
    {
        ...
        public void ExportToTextFilePath_Pass()
        {
            ...
            try
            {
                string args = string.Format(@"exporttags -o {0} -f text -l {1}",
                    Path.Combine(TestHelpers.GetPath(TestHelpers.AppPath.testOutput), @"exporttags.txt"),
                    Path.Combine(TestHelpers.GetPath(TestHelpers.AppPath.testOutput), @"log.txt"));

                File.Delete(Path.Combine(TestHelpers.GetPath(TestHelpers.AppPath.testOutput), @"exporttags.txt"));

                exitCode = (ExportTagsResult.ExitCode)Microsoft.ApplicationInspector.CLI.Program.Main(args.Split(' '));

                if (exitCode == ExportTagsResult.ExitCode.Success)
                {
                    if (!File.Exists(Path.Combine(TestHelpers.GetPath(TestHelpers.AppPath.testOutput), @"exporttags.txt")) ||
                        new FileInfo(Path.Combine(TestHelpers.GetPath(TestHelpers.AppPath.testOutput), @"exporttags.txt")).Length == 0)
                    {
        ...
        public void ExportToBadOutFilePath_Fail()
        {
            ...
            {
                string args = string.Format(@"exporttags -o {0} -f text -l {1}",
                    Path.Combine(TestHelpers.GetPath(TestHelpers.AppPath.testOutput), @"\baddir\exporttags.txt"),
                    Path.Combine(TestHelpers.GetPath(TestHelpers.AppPath.testOutput), @"log.txt"));
        ...
        public void NoConsoleOutput_Pass()
        {
            ...
            try
            {
                ...
                exitCode = (ExportTagsResult.ExitCode)TestHelpers.RunProcess(appInspectorPath, args, out string testContent);
